...
import { getSuperAdminAIAdvice } from "@/app/actions/superAdminAi";

type Message = { role: "user" | "ai"; text: string };

export default function SuperAdminAIModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  const handleAsk = async () => {
    ...
    setLoading(true);
    try {
      const res = await getSuperAdminAIAdvice(userMessage);
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 p-4">
      <div className="bg-white w-full max-w-lg rounded-[2.5rem] shadow-2xl flex flex-col" style={{ height: "600px" }}>
        ...
        <div className="flex justify-between items-center px-8 pt-7 pb-4 border-b border-gray-100 shrink-0">
          <div>
            <h2 className="text-2xl font-black text-indigo-600">SUPER AI 🌐</h2>
            <p className="text-xs text-slate-400 mt-0.5">Gyms, payments, admins aur global stats</p>
          </div>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4 space-y-3">
          {messages.length === 0 && (
            <p className="text-center text-xs text-slate-400 mt-10">
              Misal: Kitni gyms hain? | Is month revenue? | Pending admin approvals?
            </p>
          )}
          {messages.map((msg, i) => (
