...
import { getAdminAIAdvice } from "@/app/actions/adminAi";

type Message = { role: "user" | "ai"; text: string };

export default function AdminAIModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  const handleAsk = async () => {
    ...
    setLoading(true);
    try {
      const res = await getAdminAIAdvice(userMessage);
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 p-4">
      <div className="bg-white w-full max-w-lg rounded-[2.5rem] shadow-2xl flex flex-col" style={{ height: "600px" }}>
        ...
        <div className="flex justify-between items-center px-8 pt-7 pb-4 border-b border-gray-100 shrink-0">
          <div>
            <h2 className="text-2xl font-black text-indigo-600">ADMIN AI 🏋️</h2>
            <p className="text-xs text-slate-400 mt-0.5">Members, trainers, attendance aur fees</p>
          </div>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4 space-y-3">
          {messages.length === 0 && (
            <p className="text-center text-xs text-slate-400 mt-10">
              Misal: Kitne members active hain? | Ali ki fees pending hai?
            </p>
          )}
          {messages.map((msg, i) => (
